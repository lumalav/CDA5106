...
public class Main {
    ...
    public static void E1() throws Exception {
        ...
        for(int i = 0; i < 3; i++) {
            ...
            for(int j = 1; j <= 6; j++) {
                X.add((double)j);
                arguments.Load(PredictionPolicyType.Smith, j, -1, "traces\\" + files[i] + "_trace.txt")
                         .Run();
    ...
    public static void E2() throws Exception {
        ...
        for(int i = 0; i < 3; i++) {
            ...
            for(int j = 7; j <= 12; j++) {
                X.add((double)j);
                arguments.Load(PredictionPolicyType.Bimodal, j, -1, "traces\\" + files[i] + "_trace.txt")
                         .Run();
                Y.add(arguments.PredictionPolicy.GetMispredictionRate());
            }
            chart.addSeries(name, X, Y);
        ...
        for(int i = 0; i < 3; i++) {
            ...
            for(int n = 2; n <= 12; n=n+2) {
                ...
                while(m <=12) {
                    ...
                    X.add((double)m);
                    arguments.Load(PredictionPolicyType.Gshare, m, n, "traces\\" + files[i] + "_trace.txt")
                             .Run();
                    Y.add(arguments.PredictionPolicy.GetMispredictionRate()); 
                    m++;
                }
